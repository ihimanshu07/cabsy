...
async function loadDashboard() {
  ...
  document.querySelector('#admin-drivers-body').innerHTML = drivers.length ? drivers.map(item => `<tr><td><strong>${escapeHTML(item.full_name)}</strong><br>${escapeHTML(item.phone)}<br>${escapeHTML(item.email)}<br>${escapeHTML(item.city)}</td><td>${escapeHTML(item.experience)}<br>${escapeHTML(item.vehicle_type)}</td><td>${escapeHTML(item.vehicle_registration)}<br>${escapeHTML(item.license_number)}</td><td><select data-status-table="driver_applications" data-id="${item.id}" aria-label="Update driver application status">${statusOptions(item.status, ['pending', 'reviewing', 'approved', 'rejected'])}</select></td></tr>`).join('') : '<tr><td colspan="4">No driver applications yet.</td></tr>';
